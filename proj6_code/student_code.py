...
def detect_3d_box(img_path):
    ...
    while True:
        ...
        # heatmap and displacement files
...
def check_hand_inside_bounding_box(hand, pts):
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    offset = np.amin(pts[:,:3],0)
    t_pts = pts[:,:3]-offset
    t_hand = hand[:3]-offset
    if np.all(t_hand<=np.amax(t_pts,0)) and np.all(t_hand>=np.array(np.zeros(t_hand.shape))):
        inside = True
    else:
        inside = False
...
def process_video(path):
    ...
    def processFrame(sec):
        ...
        if hasFrames:
            ...
            actual_rot, actual_trans,P = perspective_n_points(initial_box_points_3d[:,:3], box_points_2d[:,:2], K)
            #P=K.dot(np.hstack((actual_rot, -actual_rot.dot(actual_trans))))
            # Step-2: Call hand_pose_img to get landmarks from img_path
            landmark1, _= hand_pose_img(img_path)
            # Step-3: Project 2d landmakrs to 3d using given projection_depth and project matrix from step-3
            pose3d_landmark = projection_2d_to_3d(P, projection_depth, landmark1)
            # Step-4: Get hand coordinate from the index 22 (0-indexed) of 3d landmarks
            hand = pose3d_landmark[22]
            # Step-5 Call draw_box_intersection and get annotated_img
            #print("debug",hand, vertices_world)
            annotated_img = draw_box_intersection(img, hand, vertices_world, bounding_boxes_chair_2d)
            ############################################################################
            #                             END OF YOUR CODE
            ############################################################################
            #print("../data/video/image"+str(count)+".jpg")
